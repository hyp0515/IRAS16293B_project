...
class radmc3d_setup:
    ...
    def get_dustalignmentcontrol(self,
      ...
      R_mesh, Theta_mesh = np.meshgrid(self.DM.r_sph, self.DM.theta_sph, indexing='ij')
      ...
      if hourglass is True:
        Br += alpha*(R_mesh**2)*(np.sin(Theta_mesh)**2)*np.cos(Theta_mesh)*np.exp(-alpha*(R_mesh**2)*(np.cos(Theta_mesh)**2))
        Btheta += alpha*(R_mesh**2)*(np.cos(Theta_mesh)**2)*np.sin(Theta_mesh)*np.exp(-alpha*(R_mesh**2)*(np.cos(Theta_mesh)**2))

      
      with open('grainalign_dir_temp_1.inp','w+') as f:
        f.write('1\n')
        f.write('%d\n'%(self.NR*self.NTheta*self.NPhi))
        for idx_phi in range(self.NPhi):
          for idx_theta in range(self.NTheta):
      ...
      with open('grainalign_dir_temp_2.inp','w+') as f:
        f.write('1\n')
        f.write('%d\n'%(self.NR*self.NTheta*self.NPhi))
        for idx_phi in range(self.NPhi):
          for idx_theta in range(self.NTheta):
            for idx_r in range(self.NR):
              f.write('%13.6e %13.6e %13.6e \n'%(Br[idx_r, idx_theta],Btheta[idx_r, idx_theta], Bphi))
      with open('grainalign_dir_temp_3.inp','w+') as f:
        f.write('1\n')
        f.write('%d\n'%(self.NR*self.NTheta*self.NPhi))
        for idx_phi in range(self.NPhi):
          for idx_theta in range(self.NTheta):
            for idx_r in range(self.NR):
              f.write('%13.6e %13.6e %13.6e \n'%(Br[idx_r, idx_theta],Btheta[idx_r, idx_theta], Bphi))
      with open('grainalign_dir_temp_4.inp','w+') as f:
        f.write('1\n')
        f.write('%d\n'%(self.NR*self.NTheta*self.NPhi))
        for idx_phi in range(self.NPhi):
          for idx_theta in range(self.NTheta):
      ...
      with open('dustkapalignfact_temp_1.inp','w+') as f:
          f.write('1\n')
          f.write('%d\n'%(nlam))
          f.write('%d\n\n'%(nang))
          for value in lam:
      ...
      with open('dustkapalignfact_temp_2.inp','w+') as f:
          f.write('1\n')
          f.write('%d\n'%(nlam))
          f.write('%d\n\n'%(nang))
          for value in lam:
          ...
          for inu in range(nlam):
              for imu in range(nang):
                  f.write('%13.6e %13.6e\n'%(k_orth[imu],k_para[imu]))
      with open('dustkapalignfact_temp_3.inp','w+') as f:
          f.write('1\n')
          f.write('%d\n'%(nlam))
          f.write('%d\n\n'%(nang))
          for value in lam:
          ...
          for inu in range(nlam):
              for imu in range(nang):
                  f.write('%13.6e %13.6e\n'%(k_orth[imu],k_para[imu]))
      with open('dustkapalignfact_temp_4.inp','w+') as f:
          f.write('1\n')
          f.write('%d\n'%(nlam))
          f.write('%d\n\n'%(nang))
          for value in lam:
